...
def check_permissions(permission, playload): #TODO BY TOMORROW ### DONE
    #Check if permission is contained in payload

    if len(playload['permissions']) == 0:
        raise AuthError({
            'code': 'invalid_claims',
            'description': 'You have no permissions'
        }, 401)

    if 'permissions' not in playload:
        print('no per')
        raise AuthError({
            'code':'invalid_claims',
            'description':'Permissions not included in JWT'
        },400)
    if permission not in playload['permissions']:
        raise AuthError({
            'code':'unauthorized',
    ...
    def requires_auth_decorator(f):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                check_permissions(permission, playload)
            except:
                abort(401)
